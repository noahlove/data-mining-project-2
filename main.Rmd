...
Here is an example of the cosine similarity matrix. Noah's resume is the same as itself so it has a similarity of 1. It is also interestingly almost nothing like Ido's (very close to 0). In this way, the similarity provides a metric for how similar or "good" the match is to our resume and provided terms.
...
Here is the output for each of us for example:
...
div_score <- mean(cossim[1,2],cossim[1,3],cossim[2,3])
