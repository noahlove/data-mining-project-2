...
long_df <- unnest(df_json20) %>% 
  pivot_longer(df_cleaned, cols = 5:574, names_to = "post_id", values_to = "description") %>%
  select(-l, -jt) %>% 
  rename(category = q)
...
### Data Quality Issue
Some companies appear to have gotten lazy in their copy and pasting between websites and have smashed some words together. Because it is a rare enough occurence, it will hopefully not have a large effect on the data. 
